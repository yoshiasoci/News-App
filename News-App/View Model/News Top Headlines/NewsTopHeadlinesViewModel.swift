//
//  NewsTopHeadlinesViewModel.swift
//  News-App
//
...
protocol NewsTopHeadlinesViewModelable {
...
class NewsTopHeadlinesViewModel: NewsTopHeadlinesViewModelable {
    ...
    private func getTopHeadlines(page: Int) {
        self.isLoading.onNext(true)
        self.isSearchArticle.accept(false)
        apiServiceProvider.request(.getTopHeadlines(apiKey: apiKey, country: "id", page: page)) { (result) in
            switch result {
    ...
    private func loadPhoto() {
        if stopLoad.value == false {
            if isSearchArticle.value == true {
                ...
                getTopHeadlines(page: self.pageArticle.value! + 1)
                self.pageArticle.accept(self.pageArticle.value! + 1)
            }
        }
    }
